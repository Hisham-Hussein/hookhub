 ...
 */
function starsAriaLabel(count: number): string {
  return `${count.toLocaleString()} GitHub stars`
}

/**
 * HookCard — A single hook displayed as a clickable card.
 *
 * Entire card is a link to the hook's GitHub repository.
 * Uses a stretched `::after` pseudo-element on the `<a>` title
 * so the whole card surface is one click/tab-stop target.
 *
 * Content hierarchy (strict order):
 * 1. Hook name (h3, most prominent)
 * 2. Category badge + event badge (sky / indigo)
 * 3. Description (2-line clamp)
 * 4. Stars count (with star icon)
  ...
  return (
    <article className="group relative rounded-xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-950 p-5 space-y-3 transition-all duration-200 hover:border-zinc-400 dark:hover:border-zinc-600 hover:shadow-lg hover:shadow-sky-500/[0.03] focus-within:ring-2 focus-within:ring-sky-400/50 focus-within:ring-offset-2 focus-within:ring-offset-white dark:focus-within:ring-offset-black">
      ...
      <div className="flex gap-2 flex-wrap">
        <span className="relative z-10 inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-light bg-sky-50 text-sky-700 border border-sky-200 dark:bg-sky-500/15 dark:text-sky-400 dark:border-sky-500/20 tracking-wide">
      ...
      {/* Description — 2-line clamp, full text accessible to screen readers */}
